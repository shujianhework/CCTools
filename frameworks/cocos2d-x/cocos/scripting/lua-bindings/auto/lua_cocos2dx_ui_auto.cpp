...
int lua_cocos2dx_ui_ImageView_PtColor(lua_State* tolua_S)
{
	int argc = 0;
	cocos2d::ui::ImageView* cobj = nullptr;
	bool ok = true;
	...
	if (!cobj)
	{
		tolua_error(tolua_S, "invalid 'cobj' in function 'lua_cocos2dx_ImageView_getPtColor'", nullptr);
		return 0;
	}
#endif
	argc = lua_gettop(tolua_S) - 1;
	do {
		cocos2d::Color3B2 ret;
		if (argc == 2) {
			double arg0;
			ok &= luaval_to_number(tolua_S, 2, &arg0, "ccui.Image:getPtColor");

			if (!ok) { break; }
			double arg1;
			ok &= luaval_to_number(tolua_S, 3, &arg1, "ccui.Image:getPtColor");

			if (!ok) { break; }
			if (cobj->getPtColor(arg0, arg1, ret, false)) {
				color3b2_to_luaval(tolua_S, ret);
				//lua_settop(tolua_S, 1);
				return 1;
			}
			else {
		...
		if (argc == 3) {
			...
			if (!ok) { break; }
			bool IsHsv = false;
			ok &= luaval_to_boolean(tolua_S, 4, &IsHsv, "ccui.Image:getPtColor");
			if (!ok)break;

			if (cobj->getPtColor(arg0, arg1, ret, IsHsv)) {
				color3b2_to_luaval(tolua_S, ret);
				//lua_settop(tolua_S, 1);
				return 1;
			}
	...
	ok = true;
	luaL_error(tolua_S, "%s has wrong number of arguments: %d, was expecting %d \n", "ccui.Image:getPtColor", argc, 1);
	return 0;

#if COCOS2D_DEBUG >= 1
tolua_lerror:
	tolua_error(tolua_S, "#ferror in function 'lua_cocos2dx_ImageView_getPtColor'.", &tolua_err);
#endif
...
int lua_register_cocos2dx_ui_ImageView(lua_State* tolua_S)
{
	...
	tolua_function(tolua_S, "getPtColor", lua_cocos2dx_ui_ImageView_PtColor);
	tolua_endmodule(tolua_S);
...
int lua_cocos2dx_ui_Text_setInvalidString(lua_State* tolua_S)
{
	int argc = 0;
	cocos2d::ui::Text* cobj = nullptr;
	bool ok = true;
	...
	if (!cobj)
	{
		tolua_error(tolua_S, "invalid 'cobj' in function 'lua_cocos2dx_ui_Text_setInvalidString'", nullptr);
		return 0;
	}
#endif
	...
	if (argc == 1)
	{
		...
		ok &= luaval_to_std_string(tolua_S, 2, &arg0, "ccui.Text:setInvalidString");
		if (!ok)
		{
			tolua_error(tolua_S, "invalid arguments in function 'lua_cocos2dx_ui_Text_setInvalidString'", nullptr);
			return 0;
		}
		bool success = cobj->setInvalidString(arg0);
		tolua_pushboolean(tolua_S, success);
		return 1;
	}
	luaL_error(tolua_S, "%s has wrong number of arguments: %d, was expecting %d \n", "ccui.Text:setInvalidString", argc, 1);
	return 0;

#if COCOS2D_DEBUG >= 1
tolua_lerror:
	tolua_error(tolua_S, "#ferror in function 'lua_cocos2dx_ui_Text_setInvalidString'.", &tolua_err);
#endif

	return 0;
}
int lua_cocos2dx_ui_Text_getInvalidCharacterSets(lua_State* tolua_S)
{
	int argc = 0;
	cocos2d::ui::Text* cobj = nullptr;
	bool ok = true;
	...
	if (!cobj)
	{
		tolua_error(tolua_S, "invalid 'cobj' in function 'lua_cocos2dx_ui_Text_getInvalidCharacterSets'", nullptr);
		return 0;
	}
#endif
	...
	if (argc == 1)
	{
		...
		ok &= luaval_to_std_string(tolua_S, 2, &arg0, "ccui.Text:getInvalidCharacterSets");
		if (!ok)
		{
			tolua_error(tolua_S, "invalid arguments in function 'lua_cocos2dx_ui_Text_getInvalidCharacterSets'", nullptr);
			return 0;
		}
		char arr[10] = "";
		//cobj->setString("");
		int len = -1;
		unsigned int* ret = cobj->getInvalidCharacterSets(arg0, len);
		if (ret == NULL) {
			lua_settop(tolua_S, 1);
			return 1;
		}
		...
		for (int i = 0; i < len; i++)
		{
			unsigned int val = ret[i];
			char16_t al = (val % 65535);
			utf16 = al;
			StringUtils::UTF16ToUTF8(utf16, utf8);
			lua_pushstring(tolua_S, utf8.c_str());
			utf8 = "";
			sprintf_s(arr, "%d", val / 65535);
			lua_setfield(tolua_S, -2, arr);
		}
		return 1;
	}
	luaL_error(tolua_S, "%s has wrong number of arguments: %d, was expecting %d \n", "ccui.Text:getInvalidCharacterSets", argc, 1);
	return 0;

#if COCOS2D_DEBUG >= 1
tolua_lerror:
	tolua_error(tolua_S, "#ferror in function 'lua_cocos2dx_ui_Text_getInvalidCharacterSets'.", &tolua_err);
#endif
...
int lua_register_cocos2dx_ui_Text(lua_State* tolua_S)
{
	...
	tolua_function(tolua_S, "setInvalidString", lua_cocos2dx_ui_Text_setInvalidString);
	tolua_function(tolua_S, "getInvalidCharacterSets", lua_cocos2dx_ui_Text_getInvalidCharacterSets);
